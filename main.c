...
int main (int argc, char **argv) {
	...
	char jobmessage[64] = "JOB,revox,LOW";
	char serverreply[128];
	int socket_desc;
	struct sockaddr_in server;
	char serverversion[3];
	char username[64] = "";
	...
	//strcat(jobmessage, ',LOW'); //combine "JOB," and duco username
	...
	while (1) {
		...
		if (recv(socket_desc, serverreply, 128, 0) < 0) {
			printf("Error: Job couldn't be received\n");
			return 1;
		}
		...
		for (int i = 0; i < (100 * atoi(diff)) + 1; i++) {
			...
			if (strcmp(work, buf) == 0) {
				...
				char feedback[8] = "";
				if (recv(socket_desc, feedback, 8, 0) < 0) { // receive feedback
